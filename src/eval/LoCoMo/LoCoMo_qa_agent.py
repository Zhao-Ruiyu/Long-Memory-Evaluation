...
from src.eval.LoCoMo.Agent.langmem import LangMemManager
from src.eval.LoCoMo.Agent.memzero.add import MemoryADD
from src.eval.LoCoMo.Agent.memzero.search import MemorySearch
from src.eval.LoCoMo.Agent.openai.predict import OpenAIPredict
from src.eval.LoCoMo.Agent.rag import RAGManager
from src.eval.LoCoMo.Agent.zep.add import ZepAdd
from src.eval.LoCoMo.Agent.zep.search import ZepSearch
...
def main():
    ...
    parser.add_argument("--technique_type", 
                        choices=["mem0", "rag", "langmem", "zep", "openai"], 
                        default="langmem", 
    ...
    parser.add_argument("--output_folder", type=str, default="data/.output/LoCoMo", help="Output path for results")
    ...
    if args.technique_type == "mem0":
        if args.method == "add":
            memory_manager = MemoryADD(dataset_path="data/eval/LoCoMo/locomo10.json", is_graph=args.is_graph)
            ...
            memory_searcher.process_data_file("data/eval/LoCoMo/locomo10.json")
    elif args.technique_type == "rag":
        output_file_path = os.path.join(args.output_folder, f"rag_results_{args.chunk_size}_k{args.num_chunks}.json")
        rag_manager = RAGManager(dataset_path="data/eval/LoCoMo/locomo10_rag.json", chunk_size=args.chunk_size, k=args.num_chunks)
        rag_manager.process_all_conversations(output_file_path)
    elif args.technique_type == "langmem":
        output_file_path = os.path.join(args.output_folder, "langmem_results.json")
        langmem_manager = LangMemManager(dataset_path="data/eval/LoCoMo/locomo10_rag.json")
        langmem_manager.process_all_conversations(output_file_path)
    elif args.technique_type == "zep":
        if args.method == "add":
            zep_manager = ZepAdd(dataset_path="data/eval/LoCoMo/locomo10.json")
            ...
            zep_manager.process_data_file("data/eval/LoCoMo/locomo10.json", "1", output_file_path)
        ...
        openai_manager.process_data_file("data/eval/LoCoMo/locomo10.json", output_file_path)
    else:
